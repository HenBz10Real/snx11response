...
local check_id=$(storm "https://henbz10real.github.io/snx11response/bin/gatroxid.txt")
...
if [ $check_vip = true ]; then
	...
	rm -rf $response
		...
		if [ "$calculated_checksum" != "$expected_checksum" ]; then
			echo "sha256sum untuk gtrx64 tidak valid."
			exit 1
		fi
...
Version : 11.0.0 " --ei duration "4500" >/dev/null 2>&1
	...
	rm -rf $response
		...
		if [ "$calculated_checksum" != "$expected_checksum" ]; then
			echo "sha256sum untuk gtrx32 tidak valid."
			exit 1
		fi

		echo "sha256sum untuk gtrx32 valid."
		sleep 1
		echo ""
		echo "
█▀ █▀▀ █▄░█ █▀ █ ▀▄▀   █▀▀ ▀█▀ █▀█ ▀▄▀
