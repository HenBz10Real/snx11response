...
versi="v11.2.0-(GATROX)"
v_toast="Version : 11.2.0"
...
▄█ ██▄ █░▀█ ▄█ █ █░█   █▄█ ░█░ █▀▄ █░█"
		...
		if [ ! "$status" ]; then
		    if curl -o /data/local/tmp/gtrx64 $url64>/dev/null 2>&1; then
...
▄█ ██▄ █░▀█ ▄█ █ █░█   █▄█ ░█░ █▀▄ █░█"
		...
		if [ ! "$status" ]; then
			if curl -o /data/local/tmp/gtrx32 $url32>/dev/null 2>&1; then
